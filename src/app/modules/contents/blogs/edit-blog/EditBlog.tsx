...
import { IBlogRequest, IBlogResponse } from "../core/models/blog.interface";
import { getBlog, updateBlog } from "../core/api/blog.request";
...
const EditBlog = () => {
  ...
  const [blog, setBlog] = React.useState<IBlogResponse | null>(null);
  ...
  React.useEffect(() => {
    if (blogId) {
      ...
      getBlog(parseInt(blogId)).then((res) => {
        setBlog(res);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      updateBlog({ id: parseInt(blogId!), data: data })
        .then(() => {
  ...
  return (
    <form className="container" onSubmit={formik.handleSubmit}>
      <Card>
        <Card.Header>
          <h4 className="text-lg font-bold">Blog Düzenle</h4>
        </Card.Header>
        <Card.Body>
            ...
            <div className="mb-3">
              ...
              <AsyncSelect
                ...
                value={{
                  ...
                  label: formik.values.category?.title,
                }}
                onChange={(data) => {
