...
const EditDynamicPage = () => {
  ...
  const { id: dynamicPageId } = useParams();
  const [dynamicPage, setDynamicPage] =
    React.useState<IDynamicPageResponse | null>(null);
  ...
  React.useEffect(() => {
    if (dynamicPageId) {
      ...
      getDynamicPage(parseInt(dynamicPageId)).then((res) => {
        setDynamicPage(res);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      short_description: dynamicPage?.short_description ?? "",
      description: dynamicPage?.description ?? "",
      meta_title: dynamicPage?.meta_title ?? "",
      meta_description: dynamicPage?.meta_description ?? "",
      main_image: dynamicPage?.main_image ?? "",
      meta_keywords: dynamicPage?.meta_keywords ?? "",
      url: dynamicPage?.url ?? "",
      active: dynamicPage?.active ?? false,
    } as IDynamicPageResponse,
    validationSchema: validationSchema,
    enableReinitialize: true,
    onSubmit: (values) => {
      updateDynamicPage({ id: parseInt(dynamicPageId!), data: values }).then(
        () => {
  ...
  return (
    <form className="container" onSubmit={formik.handleSubmit}>
      <Card>
        <Card.Header>
          <h4 className="text-lg font-bold">Sayfa Düzenle</h4>
        </Card.Header>
        <Card.Body>
