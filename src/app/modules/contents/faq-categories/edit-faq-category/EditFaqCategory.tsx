...
const EditFaqCategory = () => {
  ...
  const { id: faqCategoryId } = useParams();
  const [faqCategory, setFaqCategory] =
    React.useState<IFaqCategoryResponse | null>(null);
  ...
  React.useEffect(() => {
    if (faqCategoryId) {
      ...
      getFaqCategory(parseInt(faqCategoryId)).then((res) => {
        setFaqCategory(res);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      active: faqCategory?.active ?? false,
    } as IFaqCategoryResponse,
    validationSchema: validationSchema,
    enableReinitialize: true,
    onSubmit: (values) => {
      updateFaqCategory({ id: parseInt(faqCategoryId!), data: values })
        .then(() => {
  ...
  return (
    <form className="container" onSubmit={formik.handleSubmit}>
      <Card>
        <Card.Header>
          <h4 className="text-lg font-bold">SSS Kategori Düzenle</h4>
        </Card.Header>
        <Card.Body>
