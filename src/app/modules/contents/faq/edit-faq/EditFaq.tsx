...
import { IFaqRequest, IFaqResponse } from "../core/models/faq.interface";
import { getFaq, updateFaq } from "../core/api/faq.request";
...
const validationSchema = Yup.object().shape({
  ...
  faq_category: Yup.object().shape({
    id: Yup.number().required("Kategori zorunludur"),
    title: Yup.string().required("Kategori zorunludur"),
  }),
});
const EditFaq = () => {
  const navigate = useNavigate();
  const { id: faqId } = useParams();
  const [faq, setFaq] = React.useState<IFaqResponse | null>(null);
  ...
  React.useEffect(() => {
    if (faqId) {
      ...
      getFaq(parseInt(faqId)).then((res) => {
        setFaq(res);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      faq_category: faq?.faq_category ?? null,
    } as IFaqResponse,
    ...
    onSubmit: (values) => {
      const valuesToSend: IFaqRequest = {
        ...
        faq_category_id: values.faq_category?.id ?? 0,
      };
      updateFaq({ id: parseInt(faqId!), data: valuesToSend }).then(() => {
  ...
  return (
    <form className="container" onSubmit={formik.handleSubmit}>
      <Card>
        <Card.Header>
          <h4 className="text-lg font-bold">SSS Güncelle</h4>
        </Card.Header>
        <Card.Body>
          <div className="max-w-3xl mx-auto p-10">
            <div className="mb-3">
              ...
              <AsyncSelect
                isSearchable
                value={{
                  value: formik.values.faq_category?.id,
                  label: formik.values.faq_category?.title,
                }}
                onChange={(data) => {
                  formik.setFieldValue("faq_category", {
                    id: data?.value,
                    title: data?.label,
                  });
                }}
              ...
              {formik.touched.faq_category && formik.errors.faq_category ? (
                <p className="mt-2 text-sm text-red-500">
                  {formik.errors.faq_category?.title}
                </p>
              ) : null}
            </div>
            <div className="mb-3">
