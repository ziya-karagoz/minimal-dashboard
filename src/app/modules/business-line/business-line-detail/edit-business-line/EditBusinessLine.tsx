...
import { fetchBusinessLine, updateBusinessLine } from "../../core/api/business-line.requests";

const validationSchema = Yup.object().shape({
    name: Yup.string().required("İş Kolu adı zorunludur"),
    description: Yup.string().required("İş Kolu açıklaması zorunludur"),
});

const EditBusinessLine = () => {
    const { id: businessLineId } = useParams();
    const [businessLine, setBusinessLine] = React.useState<IBusinessLineResponseP | null>(null);
    const [fetchStatus, setFetchStatus] = React.useState<FetchStatus>(FetchStatus.IDLE);

    useEffect(() => {
        if (businessLineId) {
            setFetchStatus(FetchStatus.LOADING);
            fetchBusinessLine(parseInt(businessLineId)).then((res) => {
                setBusinessLine(res);
    ...
    const formik = useFormik({
        initialValues: {
            ...
            active: businessLine?.active || false,
            description: businessLine?.description || "",
        } as IBusinessLineResponseP,
        validationSchema: validationSchema,
        enableReinitialize: true,
        onSubmit: (values) => {
            updateBusinessLine({ id: parseInt(businessLineId!), data: values }).then(() => {
    ...
    return (
        <form onSubmit={formik.handleSubmit}>
            <Card>
                <Card.Header>
                    <h4 className="text-lg text-gray-800 font-bold">{businessLine?.name} İş Kolunu Düzenle</h4>
                </Card.Header>
                <Card.Body>
